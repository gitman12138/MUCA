...
from networks.unet_cpl import UNet_cpl
import argparse
from networks.vision_transformer import SwinUnet as ViT_seg
from networks.vit_seg_modeling import VisionTransformer as ViT_cca
...
def get_r50_b16_config(n_classes):
...
def net_factory(net_type="unet", in_chns=1, class_num=3):
    config_cca = get_r50_b16_config(class_num)
    if net_type == "unet":
        net = UNet(in_chns=in_chns, class_num=class_num).cuda()
    elif net_type == "unet_icl":
        net = UNet_icl(in_chns=in_chns, class_num=class_num).cuda()
    elif net_type == "unet_cpl":
        net = UNet_cpl(in_chns=in_chns, class_num=class_num).cuda()
    elif net_type == "vit_cca":
        net = ViT_cca(config=config_cca, n_classes=class_num).cuda()
