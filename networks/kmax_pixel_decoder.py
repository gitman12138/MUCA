...
class RelativePositionalEncoding(nn.Module):
    ...
    def forward(self):
        return self._embeddings.weight[self._relative_distance_matrix.reshape(-1)].reshape(self.query_length,
...
class AxialAttention(nn.Module):
    def __init__(self, in_planes, query_shape=56, total_key_depth=512, total_value_depth=1024, num_heads=8):
        ...
        self.qkv_transform = ConvBN(in_planes, self._total_key_depth * 2 + self._total_value_depth, kernel_size=1,
                                    stride=1,
        ...
        self._batch_norm_qkv = get_norm('1b', self._total_key_depth * 2 + self._total_value_depth)
        self._batch_norm_similarity = get_norm('bn', num_heads * 3)
        self._batch_norm_retrieved_output = get_norm('1b', self._total_value_depth * 2)

    def forward(self, x):
        N, C, L = x.shape
        # qkv = self._batch_norm_qkv(self.qkv_transform(x))
        temp = self.qkv_transform(x)
        qkv = self._batch_norm_qkv(temp)
        ...
        similarity_logits = self._batch_norm_similarity(similarity_logits).reshape(N, 3, self._num_heads, L, L).sum(
            dim=1)
        ...
        retrieved_output = torch.cat([retrieved_content, retrieved_rpe], dim=1).reshape(N, 2 * self._total_value_depth,
                                                                                        L)
        retrieved_output = self._batch_norm_retrieved_output(retrieved_output).reshape(N, 2, self._total_value_depth,
                                                                                       L).sum(1)
...
class SingleBlock(nn.Module):
    ...
    def __init__(self, inplanes, filter_list, block_type, query_shape=[56, 56], key_expansion=1, value_expansion=2,
                 num_heads=8, drop_path_prob=0.0):
        super(SingleBlock, self).__init__()
        self._block_type = block_type.lower()
        self._filter_list = filter_list
        self._conv1_bn_act = ConvBN(inplanes, self._filter_list[0], kernel_size=1, bias=False, norm='bn', act='gelu')
        if self._block_type == 'axial':
            self._attention = AxialAttention2D(in_planes=self._filter_list[0], query_shape=query_shape,
                                               filters=self._filter_list[1],
                                               key_expansion=key_expansion, value_expansion=value_expansion,
                                               num_heads=num_heads)
            output_channel = filter_list[1] * value_expansion
        elif self._block_type == 'bottleneck':
            self._conv2_bn_act = ConvBN(self._filter_list[0], self._filter_list[1], kernel_size=3, padding=1,
                                        bias=False, norm='bn', act='gelu')
            output_channel = filter_list[1]
        self._conv3_bn = ConvBN(output_channel, self._filter_list[2], kernel_size=1, bias=False, norm='bn', act=None,
                                norm_init=0.0)

        self._shortcut = None
        if inplanes != self._filter_list[-1]:
            self._shortcut = ConvBN(inplanes, self._filter_list[-1], kernel_size=1, bias=False, norm='bn', act=None)
...
class BlockGroup(nn.Module):
    def __init__(self, inplanes, base_filter, num_blocks, block_type, **kwargs):
        ...
        for i in range(num_blocks):
            self._blocks.append(
                SingleBlock(inplanes=inplanes, filter_list=filter_list, block_type=block_type, **kwargs))
...
class ResizedFuse(nn.Module):
    def __init__(self, low_in_channels, high_in_channels, out_channels):
        ...
        if low_in_channels != out_channels:
            self._conv_bn_low = ConvBN(low_in_channels, out_channels, kernel_size=1, bias=False, norm='bn', act=None)
        if high_in_channels != out_channels:
            self._conv_bn_high = ConvBN(high_in_channels, out_channels, kernel_size=1, bias=False, norm='bn', act=None)
...
class kMaXPixelDecoder(nn.Module):
    ...
    def __init__(
        ...
        assert self.num_stages == len(dec_layers) and self.num_stages == len(dec_channels) and self.num_stages == len(
            layer_types)
        ...
        for i in range(self.num_stages):
            ...
            self._stages.append(BlockGroup(inplanes=inplanes,
                                           base_filter=dec_channels[i], num_blocks=dec_layers[i],
                                           block_type=layer_types[i],
                                           query_shape=query_shape[i], key_expansion=1, value_expansion=2, num_heads=8,
    ...
    def forward_features(self, features):
        ...
        # OS 32, 8, 4
        semantic_features = [features[self.in_features[0]], features[self.in_features[2]],
